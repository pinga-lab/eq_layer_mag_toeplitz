...
In order to describe matrix components $\mathbf{A_{\boldsymbol{\alpha\beta}}}$ (equation
\ref{eq:A_expand}) in terms of block indices, we have to verify that:
(i) each pair of indices $i$ and $j$ produce a pair of block indices $q$ and $p$;
(ii) each element $a^{\alpha\beta}_{ij}$, defined by indices $i$ and $j$, can be 
represented by the alternative notation $a^{\alpha\beta}_{qp}$, defined in terms of
the associated block indices $q$ and $p$;
(iii) different combinations of indices $i$ and $j$ result in the same block indices $q$ and $p$; 
(iv) elements $a^{\alpha\beta}_{ij}$ having the same associated block indices $q$ and $p$
also have the same numerical value;
(v) matrix $\mathbf{A}_{\boldsymbol{\alpha\beta}}$ is formed by a grid of $Q \times Q$ blocks
$\mathbf{A}_{\boldsymbol{\alpha\beta}}^{q}$, where each block has $P \times P$ elements
$a^{\alpha\beta}_{qp}$ with the same block index $q$;
(vi) blocks $\mathbf{A}_{\boldsymbol{\alpha\beta}}^{q}$ disposed along the same block diagonal 
of $\mathbf{A}_{\boldsymbol{\alpha\beta}}$ have the same block index $q$ and are equal to each other;
(vii) in each block $\mathbf{A}_{\boldsymbol{\alpha\beta}}^{q}$, the elements $a^{\alpha\beta}_{qp}$
having the same block index $p$ are arranged along the same diagonal and are equal to each other.

Item (i) is a direct consequence of evaluating equations \ref{eq:q-x-oriented}--\ref{eq:p-y-oriented}
for a given pair of indices $i$ and $j$. Item (ii) can be verified by substituting
the block indices $q$ and $p$ (equations \ref{eq:q-x-oriented}--\ref{eq:p-y-oriented}) 
into the second derivatives $h^{\alpha\beta}_{ij}$ 
(equations \ref{eq:hxx_regular}--\ref{eq:hyz_regular}) and then use them to
define the elements $a^{\alpha\beta}_{ij}$ (equation \ref{eq:aij_alpha_beta}). The result
will be the elements $a^{\alpha\beta}_{qp}$. Item (iii) can be shown by first verifying that
different combinations of indices $i$ and $j$ result in the same integer functions
$\Delta k_{ij}$ and $\Delta l_{ij}$ (equations \ref{eq:Delta_kij} and \ref{eq:Delta_lij}).
Then, by using equations \ref{eq:q-x-oriented}--\ref{eq:p-y-oriented}, we conclude that
different combinations of $i$ and $j$ result in the same indices $q$ and $p$.
Item (iv) can be shown by noting that, according to equations 
\ref{eq:q-x-oriented}--\ref{eq:p-y-oriented}, elements $a^{\alpha\beta}_{ij}$ having the same 
associated block indices $q$ and $p$ also have the same associated integer functions 
$\Delta k_{ij}$ and $\Delta l_{ij}$ (equations \ref{eq:Delta_kij} and \ref{eq:Delta_lij}).
As a consequence, the associated second derivatives $h^{\alpha\beta}_{ij}$ 
(equations \ref{eq:hxx_regular}--\ref{eq:hyz_regular}) have the same numerical values
and elements $a^{\alpha\beta}_{ij}$ as well. Items (v)-(vii) can be shown by computing the 
block indices $q$ and $p$ (equations \ref{eq:q-x-oriented}--\ref{eq:p-y-oriented}) for a small 
data grid formed by, say, $N_{x} = 3$, $N_{y} = 2$, and generalizing the result for larger grids.

From equations \ref{eq:q-x-oriented}--\ref{eq:p-y-oriented}, we can show that
the block index $q$ assume integer values varying from $-Q+1$ to $Q-1$, while 
$p$ varies from $-P+1$ to $P-1$. Positive (negative) indices $q$ are associated with blocks
$\mathbf{A}_{\boldsymbol{\alpha\beta}}^{q}$ located below (above) the main block diagonal 
of $\mathbf{A}_{\boldsymbol{\alpha\beta}}$.
Similarly, positive (negative) indices $p$ are associated with elements $a^{\alpha\beta}_{qp}$ 
...
assumed by matrix components $\mathbf{A_{\boldsymbol{\alpha\beta}}}$ 
...
The integer functions $\Delta k_{ij}$ and $\Delta l_{ij}$ (equations \ref{eq:Delta_kij} and
\ref{eq:Delta_lij}) play a crucial role in defining the individual structures of matrix components 
$\mathbf{A}_{\boldsymbol{\alpha\beta}}$ (equation \ref{eq:BTTB_A_alpha_beta}).
These integer functions may assume positive and negative values depending on the relative
position of the observation point $(x_{i}, y_{i}, z_{0})$ and the equivalent source 
$(x_{j}, y_{j}, z_{c})$. 
%It is important noting that the structure of a given matrix component 
%$\mathbf{A}_{\boldsymbol{\alpha\beta}}$ is controlled by the numerator of the second derivatives
%$h^{\alpha\beta}_{ij}$ (equations \ref{eq:hxx_regular}--\ref{eq:hyz_regular}),
%$\alpha, \beta = x, y, z$, which are defined by the integer functions $\Delta k_{ij}$ and 
%$\Delta l_{ij}$ (equations \ref{eq:Delta_kij} and \ref{eq:Delta_lij}).
...
As a consequence, $a^{xx}_{ij} = a^{xx}_{ji}$ and $\mathbf{A}_{\boldsymbol{xx}}$ is symmetric.
...
\ref{eq:p-x-oriented} and $a^{xx}_{qp}$ can be written as follows:
\begin{equation}
...
\ref{eq:p-y-oriented} and $a^{xx}_{qp}$ can be written as follows:
\begin{equation}
a^{xx}_{qp} = c_{m} \, \frac{\mu_{0}}{4\pi} 
...
$a^{xx}_{qp} = a^{xx}_{(-q)p}$ and that $a^{xx}_{qp} = a^{xx}_{q(-p)}$ for any grid orientation.
It means that the blocks $\mathbf{A}_{\boldsymbol{xx}}^{q} = \mathbf{A}_{\boldsymbol{xx}}^{(-q)}$
and that they are symmetric. From this results we conclude the matrix component 
...
$\mathbf{A}_{\boldsymbol{zz}}$ also have this structure.
